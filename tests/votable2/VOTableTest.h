...
class VOTableTest : public CppUnit::TestFixture {
    ...
    public:
        ...
        void testXML() {
            ...
            ss.seekg(0, ios::beg);
            const VOTable2 vot2 = VOTable2::fromXML(ss);
            ...
            // Verify Resource
            const VOTableResource2 res1 = vot1.getResource()[0];
            const VOTableResource2 res2 = vot2.getResource()[0];
            CPPUNIT_ASSERT_EQUAL(1ul, res2.getTables().size());

            const VOTableTable2 vottab1 = res1.getTables()[0];
            const VOTableTable2 vottab2 = res2.getTables()[0];
            CPPUNIT_ASSERT(vottab1.getName() == vottab2.getName());
            CPPUNIT_ASSERT(vottab1.getDescription() == vottab2.getDescription());

            // Verify groups
            const std::vector<VOTableGroup2> groups = vottab2.getGroups();
            CPPUNIT_ASSERT_EQUAL(1ul, groups.size());
            CPPUNIT_ASSERT_EQUAL(2ul, groups[0].getFieldRefs().size());
            const std::vector<VOTableParam2> params = groups[0].getParams();
            ...
            // Verify fields
            const std::vector<VOTableField2> fields = vottab2.getFields();
            ...
            // Verify rows
            const std::vector<VOTableRow2> rows = vottab2.getRows();
            CPPUNIT_ASSERT_EQUAL(2ul, fields.size());
    ...
    private:
        ...
        static VOTable2 makeTable() {
            VOTableTable2 vottab;
