...
/// @details This class extends further DDCalBufferDataAccessor adapter by
...
DDCalOnDemandNoiseAndFlagDA::DDCalOnDemandNoiseAndFlagDA(const IConstDataAccessor &acc) :
      MetaDataAccessor(acc), DDCalBufferDataAccessor(acc),
...
const casacore::Cube<casacore::Complex>& DDCalOnDemandNoiseAndFlagDA::noise() const
{
...
///         complex noise estimates. Elements correspond to the
///         visibilities in the data cube.
casacore::Cube<casacore::Complex>& DDCalOnDemandNoiseAndFlagDA::rwNoise()
{
  if (!itsNoiseSubstituted) {
...
const casacore::Cube<casacore::Bool>& DDCalOnDemandNoiseAndFlagDA::flag() const
{
...
/// @return a reference to nPol x nChannel x nRow cube with the flag
///         information. If True, the corresponding element is flagged.
casacore::Cube<casacore::Bool>& DDCalOnDemandNoiseAndFlagDA::rwFlag()
{
  if (!itsFlagSubstituted) {
