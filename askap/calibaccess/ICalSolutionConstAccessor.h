...
struct ICalSolutionConstAccessor {
   ...
   /// @return true, if the matrix returned by jones(...) method called with the same parameters is
   /// valid, false otherwise
   bool jonesAllValid(const casacore::uInt ant, const casacore::uInt beam, const casacore::uInt chan) const;

   std::pair<casa::SquareMatrix<casa::Complex, 2>, bool> jonesAndValidity(const JonesIndex &index,
       const casa::uInt chan) const;

   std::pair<casa::SquareMatrix<casa::Complex, 2>, bool> jonesAndValidity(const casa::uInt ant,
